...
function matrix:det()
    ...
    assert(#self == #self[1], "matrix not square")
    ...
    if size == 3 then
        return (self[1][1] * self[2][2] * self[3][3] + self[1][2] * self[2][3] * self[3][1] + self[1][3] * self[2][1] * self[3][2]
                - self[1][3] * self[2][2] * self[3][1] - self[1][1] * self[2][3] * self[3][2] - self[1][2] * self[2][1] * self[3][3])
    end
...
function matrix:dogauss()
            ...
            for i = j + 1, rows do
                ...
                if self[i][j] ~= zero then
                    ...
                    for _j = j + 1, columns do
                        ...
                        self[i][_j] = self[i][_j] - factor * self[j][_j]
                    end
                end
            end
        else
    ...
    for j = rows, 1, -1 do
        ...
        local div = self[j][j]
        for _j = j + 1, columns do
            self[j][_j] = self[j][_j] / div
        end
        -- start parsing rows
        for i = j - 1, 1, -1 do
            -- check if element is not already zero
            if self[i][j] ~= zero then
                local factor = self[i][j]
                for _j = j + 1, columns do
                    self[i][_j] = self[i][_j] - factor * self[j][_j]
                end
                self[i][j] = copy(zero)
