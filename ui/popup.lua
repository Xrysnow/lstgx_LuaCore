...
function M.pop(parent, widget, color_bg)
    ...
    ly
            ...
            :setLocalZOrder(-2)
    local mask = require('ui.button').ButtonNull(sz)
    mask:addTo(widget)
...
function M.popConfirm(parent, msg, cb)
    ...
    cb = cb or std.fvoid
    local btn1 = require('ui.button').Button1(cc.size(72, 22), function()
        w:removeSelf()
        cb(true)
    end)
    ...
    local btn2 = require('ui.button').Button1(cc.size(72, 22), function()
        w:removeSelf()
        cb(false)
    end)
