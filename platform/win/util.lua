...
function M.SetLastError(code)
    k32.SetLastError(code)
end
...
local function get_error_message(id)
    ...
    local sz = k32.FormatMessageA(M.FORMAT_MESSAGE_FROM_SYSTEM, nil, id, 0, buf, bufsize, nil)
...
M.SEM_NOGPFAULTERRORBOX = 0x0002
M.SEM_NOALIGNMENTFAULTEXCEPT = 0x0004
M.SEM_NOOPENFILEERRORBOX = 0x8000

function M.SetErrorMode(mode)
    return k32.SetErrorMode(M.flags(mode))
end
...
function M.retwith(valid)
    --nil,err-returning variant
    return function(ret)
...
function M.checkwith(valid)
    --error raising variant
    local retfunc = M.retwith(valid)
    return function(ret)
...
local function valid(ret)
    return ret, ret, 'error'
end
local function validz(ret)
    return ret == 0, true, 'zero expected, got non-zero'
end
local function validnz(ret)
    return ret ~= 0, ret, 'non-zero expected, got zero'
end
local function validnzb(ret)
    return ret ~= 0, true, 'non-zero expected, got zero'
end
local function validtrue(ret)
    return ret == 1, true, '1 (TRUE) expected, got 0 (FALSE)'
end
local function validh(ret)
    return ret ~= nil, ret, 'non NULL value expected, got NULL'
end
local function validpoz(ret)
    return ret >= 0, ret, 'positive number expected, got negative'
end

local retwith = M.retwith
local checkwith = M.checkwith

--common return-value nil,err-returning checkers.
M.retz = retwith(validz)     --a not-zero is an error
M.retnz = retwith(validnz)    --a zero is an error
M.retnzb = retwith(validnzb)   --a zero is an error, otherwise means true
M.rettrue = retwith(validtrue)  --non-TRUE is an error
M.reth = retwith(validh)     --a null pointer is an error (also converts NULL->nil)
M.retpoz = retwith(validpoz)   --a (strictly) negative number is an error

--common return-value error-raising checkers.
M.checkz = checkwith(validz)     --a not-zero is an error, a zero is true
M.checknz = checkwith(validnz)    --a zero is an error
M.checknzb = checkwith(validnzb)   --a zero is an error, a non-zero is true
M.checktrue = checkwith(validtrue)  --non-TRUE is an error
M.checkh = checkwith(validh)     --a null pointer is an error (also converts NULL->nil)
M.checkpoz = checkwith(validpoz)   --a (strictly) negative number is an error
M.check = checkwith(valid)      --generic checker
...
local function callwith2(valid)
    return function(f, ...)
        M.SetLastError(0)
        local ret = f(...)
        ret = M.ptr(ret) --discard NULL pointers
        local valid_for_sure, err = valid(ret)
        if not valid_for_sure then
...
M.callnz2 = callwith2(validnz) --EnumClipboardFormats() is a candidate for this.
M.callh2 = callwith2(validh)

--garbage collection ---------------------------------------------------------

--own an object by assigning it a finalizer.
--you should own all objects that winapi doesn't own to avoid leaking.
function M.own(o, finalizer)
...
function M.pin(resource, target)
...
function M.flags(s)
    ...
    local x = 0
    for flag in s:gmatch '[_%w]+' do
        --any separator works.
        flag = flag:upper()
        flag = assert(M[flag], 'invalid flag %s', flag)
        x = bor(x, flag)
    end
    flags_cache[s] = x
    return x
...
function M.setbits(over, mask, bits)
