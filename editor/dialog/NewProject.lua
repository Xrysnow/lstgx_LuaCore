...
local M = class('editor.dialog.NewProject', require('cc.ui.DialogBase'))
...
function M:ctor()
    ...
    for i, v in ipairs({ 'Empty', 'Single Stage', 'Single Spell Card', 'TouHou' }) do
        local rb = require('cc.ui.prefab.RadioButton').create(cc.size(20, 20))
        local yy = 168 - (i - 1) * 22
        rb:addTo(wi):setPosition(128, yy)
        group:addRadioButton(rb)
        local lb = require('cc.ui.label').create(v)
    ...
    local fp = require('cc.ui.FilePicker')('luastg', nil, true)
    fp:addTo(wi):setPosition(114, 56)
    require('cc.ui.helper').addFrame(fp:getEditBox(), cc.c3b(122, 122, 122), 1)
    self._fp = fp
    ...
    local hint = require('cc.ui.label').create('1. Choose a template.\n2. Specify file path and name.')
