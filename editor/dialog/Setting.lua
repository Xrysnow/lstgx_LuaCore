...
local M = class('editor.dialog.Setting', require('ui.DialogBase'))
...
function M:ctor()
    ...
    require('ui.label').create('Resolution X'):addTo(wi):alignLeft(16):alignBottom(155)
    require('ui.label').create('Resolution Y'):addTo(wi):alignLeft(16):alignBottom(123)
    ...
    local eb1 = require('ui.prefab.EditBox').Integer(cc.size(48, 18), 0)
    eb1:addTo(wi):alignLeft(120):alignBottom(155)
    require('ui.helper').addFrame(eb1, cc.c3b(122, 122, 122), 1)
    self._eb1 = eb1

    local eb2 = require('ui.prefab.EditBox').Integer(cc.size(48, 18), 0)
    eb2:addTo(wi):alignLeft(120):alignBottom(123)
    require('ui.helper').addFrame(eb2, cc.c3b(122, 122, 122), 1)
    self._eb2 = eb2

    local cb1 = require('ui.checkbox').createBase()
    cb1:addTo(wi):alignLeft(16):alignBottom(90)
    self._cb1 = cb1
    require('ui.checkbox').addLabel(cb1, 'Run in window mode')

    local cb2 = require('ui.checkbox').createBase()
    cb2:addTo(wi):alignLeft(16):alignBottom(60)
    self._cb2 = cb2
    require('ui.checkbox').addLabel(cb2, 'Cheat')
