...
local M = class('editor.dialog.InputTypeName', require('cc.ui.DialogBase'))
...
function M:ctor()
    ...
    require('cc.ui.label').create('Type name'):addTo(wi):alignLeft(10):alignTop(36)
    require('cc.ui.label').create('Difficulty'):addTo(wi):alignLeft(10):alignTop(36 + 32)

    local input = require('cc.ui.prefab.EditBox').String(cc.size(192, 22))
    input:addTo(wi):alignLeft(84):alignTop(36)
    self._input = input
    require('cc.ui.helper').addFrame(input, cc.c3b(122, 122, 122))

    self._difs = { 'All', 'Easy', 'Normal', 'Hard', 'Lunatic' }
    local sel = require('cc.ui.DropDownList').createBase(192, 22, self._difs, 1)
    sel:addTo(wi):alignLeft(84):setPositionY(52)
    self._sel = sel
    require('cc.ui.helper').addFrame(sel.button, cc.c3b(122, 122, 122))
